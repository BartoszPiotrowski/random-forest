...
module Make = functor (Data : DATA) -> struct
    ...
    type 'a tree =
        ...
        | Leaf of float * ('a Data.examples)

    let leaf example =
        Leaf (float_of_int (Data.label example), [example])
    ...
    let make_new_node examples =
        ...
        if Data.is_empty examples_l || Data.is_empty examples_r then
            let labels = List.map float_of_int (Data.labels examples) in
            let label = Utils.avg labels in
            Leaf(label, examples)
        else
            let labels_l = List.map float_of_int (Data.labels examples_l) in
            let labels_r = List.map float_of_int (Data.labels examples_r) in
            let label_l = Utils.avg labels_l in
            let label_r = Utils.avg labels_r in
            Node(rule, Leaf(label_l, examples_l), Leaf(label_r, examples_r))
