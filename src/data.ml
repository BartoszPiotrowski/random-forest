...
let gini_rule examples =
    ...
    let (best_fea, best_impur), (second_best_fea, second_best_impur) =
        match feas_impurs_sorted with
        | [] | [_] -> raise List_of_impurs_too_short
        | (f1, i1) :: (f2, i2) :: _ -> ((f1, i1), (f2, i2)) in
    (second_best_impur -. best_impur),
    fun example ->
